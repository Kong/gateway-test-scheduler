...
const runner = async (
  ...
  const runTest = async (test) => {
    ...
    try {
      ...
      const command = `${setupVenv} bin/busted --helper=spec/busted-ci-helper.lua -o hjtest --Xoutput "${xmlOutputFolder}/${Date.now()}.xml" ${excludeTagsOption} ${extraBustedOptions} "${filename}"`
